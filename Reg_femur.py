...
wkdir = r'E:\MicroCT data\Yoda1 small batch\Tibia Femur fully seg'
masterdir = os.path.join(wkdir,'Registered femur week 3')
masteroutput = os.path.join(wkdir,'VOI450-590_Registered femur week 3')
...
for file in sorted(os.listdir(masterdir)):
    ...
    if re.search(r'\d{3} week \d (left|right) femur',file):
        ...
        tar_img = imreadseq_multithread(os.path.join(masterdir,file),sitkimg=False,z_range=[410,630], rmbckgrd=60)
